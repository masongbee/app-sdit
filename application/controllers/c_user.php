...
class C_user extends CI_Controller {
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_user->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_user->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='user.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('user.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_user', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "user";
		$print_view=$this->load->view("p_user.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/user.html","w+");
