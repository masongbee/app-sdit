...
class C_siswa extends CI_Controller {
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_siswa->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.siswa]
		 */
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_siswa->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='siswa.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('siswa.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_siswa', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "siswa";
		$print_view=$this->load->view("p_siswa.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/siswa.html","w+");
