...
class C_mapelkelas extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_mapelkelas', '', TRUE);
	}
	...
	function getAllKelas(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mapelkelas->getAllKelas();
	...
	function getAllMapel(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mapelkelas->getAllMapel();
		echo json_encode($result);
	}
	...
	function getAllGuru(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mapelkelas->getAllGuru();
		echo json_encode($result);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mapelkelas->getAll($thn_pelajaran, $kelas, $start, $page, $limit);
		echo json_encode($result);
	}
	...
	function save(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mapelkelas->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mapelkelas->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='mapelkelas.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('mapelkelas.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_mapelkelas', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "mapelkelas";
		$print_view=$this->load->view("p_mapelkelas.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/mapelkelas.html","w+");
