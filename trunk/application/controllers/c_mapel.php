...
class C_mapel extends CI_Controller {
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mapel->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.mapel]
		 */
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mapel->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='mapel.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('mapel.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_mapel', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "mapel";
		$print_view=$this->load->view("p_mapel.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/mapel.html","w+");
