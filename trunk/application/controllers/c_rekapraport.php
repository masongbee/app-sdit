...
class C_rekapraport extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_rekapraport', '', TRUE);
	}
	...
	function getAllMapel(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_rekapraport->getAllMapel();
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_rekapraport->getAll($kelas, $thn_pelajaran);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='rekapraport.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('rekapraport.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_rekapraport', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "rekapraport";
		$print_view=$this->load->view("p_rekapraport.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/rekapraport.html","w+");
