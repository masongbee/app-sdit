...
class C_kelas extends CI_Controller {
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_kelas->getAll($start, $page, $limit);
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_kelas->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='kelas.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('kelas.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_kelas', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "kelas";
		$print_view=$this->load->view("p_kelas.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/kelas.html","w+");
